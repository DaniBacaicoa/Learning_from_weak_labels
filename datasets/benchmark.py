...
class Bk_Dataset(Dataset):
    ...
    def get_dataloader(self, indices=None):
        ...
        if self.weak_labels is None:
            dataset = torch.utils.data.TensorDataset(self.train_dataset.data[indices], self.train_dataset.targets[indices])
        else:
            dataset = torch.utils.data.TensorDataset(self.train_dataset.data[indices], self.weak_labels[indices], self.train_dataset.targets[indices],indices)

        self.train_loader = torch.utils.data.DataLoader(dataset, batch_size=self.batch_size, shuffle=self.shuffle, num_workers=0)
